    ...
    <div id="mainContent" style="height: 700px; overflow-y: auto; overflow-x: visible;">
        <div class="route-animated" >
            <div class="query-form">
                <el-button type="success" size="mini" @click="showDialog" >添加违禁词</el-button>
            </div>
            <el-table
                    :data="tableData"
                    border
    ...
    export default{
        ...
        methods: {
            getData:function () {
                ...
                window.axios.get('/disable', { params: query }).then(function (response) {
            ...
            submit:function(form){
                ...
                _this.$refs[form].validate((valid) => {
                    if(valid) {
                        ...
                        if(_this.form.id==''){
                            window.axios.post('/disable', _this.form).then(function (response) {
                                let res = response.data;
                            ...
                            window.axios.put('/disable/'+_this.form.id, _this.form).then(function (response) {
                                let res = response.data;
                                _this.editDialogVisible = false;
                                _this.formLoading = false;
                                _this.$message({
            ...
            handleDelete(index, row) {
                ...
                _this.$confirm('确定要删除这条数据？', '提示', {type: 'warning'}).then( () => {
                    _this.listLoading = true;
                    window.axios.delete('/disable/destroy', { data: param}).then(function (response) {
                        let res = response.data;
                        if(res.status === 0 ) {
