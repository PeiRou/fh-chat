...
//                $redis->flushdb();       //服务一启动就要清除之前的聊天室redis
    ...
    //检查抢到红包消息
    private function chkHongbaoNum($room_id){
        $redis = Redis::connection();
        $redis->select(1);
        $rsKeyH = 'hbN';
        if(!$redis->exists($rsKeyH.'ing')){
            $redis->setex($rsKeyH.'ing',10,'on');
            $iHongBaoNum = $this->updAllkey($rsKeyH,$room_id);     //红包异动
            //检查抢到红包消息
            foreach ($iHongBaoNum as $keyhb =>$checkHb) {
                $this->delAllkey($keyhb);
                error_log(date('Y-m-d H:i:s',time())." 抢到红包消息every=> ".$rsKeyH.'|'.$keyhb.'==='.$checkHb.'++++'.json_encode($iHongBaoNum).PHP_EOL, 3, '/tmp/chat/hongbaoNum.log');
                $iRoomInfo = $this->getUsersess($checkHb,'','hongbaoNum');     //包装计划消息
    ...
    //检查计画任务
    private function chkPlan($room_id){
        $redis = Redis::connection();
        $redis->select(1);
        $rsKeyH = 'pln';
        $iPlan = $this->updAllkey($rsKeyH,$room_id);     //计画任务
        //检查计划消息
        foreach ($iPlan as $keyHis =>$valHis) {
            $this->delAllkey($keyHis);
            error_log(date('Y-m-d H:i:s',time())." 计划发消息every=> ".$rsKeyH.'|'.$keyHis.'==='.$valHis.'++++'.json_encode($iPlan).PHP_EOL, 3, '/tmp/chat/plan.log');
    ...
    private function chkSendC($room_id){
        ...
        foreach ($isendC as $keyHis =>$valHis) {
            ...
            $usr = $redis->HGET($this->chatkey,'usr:'.md5($keyHis));
            if(empty($usr))
                continue;
    ...
    private function getMyserf($iSess){
        $redis = Redis::connection();
        $redis->select(1);
        $res = empty($redis->get($iSess))?'':(array)json_decode($redis->get($iSess));
        return $res;
    }
    ...
    private function getUserInfo($fd){
        $redis = Redis::connection();
        $redis->select(1);
        if($redis->HEXISTS($this->chatkey,'usr'.$fd)){
            $tmp = $redis->HGET($this->chatkey,'usr'.$fd);
            if(!$this->is_not_json($tmp))
                return (array)json_decode($redis->HGET($this->chatkey,'usr'.$fd));   //如果房客存在，把用户组反序列化
            else
                return '';
        }else
            return '';
    ...
    */
    ...
    private function getIdToUserInfo($k){
        $redis = Redis::connection();
        $redis->select(1);
        $tmpUsr = $redis->HGET($this->chatkey,'usr:'.$k)?$redis->HGET($this->chatkey,'usr:'.$k):'';                      //从md5的用户ID去找到在聊天室的广播号码
        $tmpUsrInfo = empty($tmpUsr) || !$redis->HEXISTS($this->chatkey,$tmpUsr)?'':(array)json_decode($redis->HGET($this->chatkey,$tmpUsr));     //从聊天室的广播号码取得每个人的聊天室信息
        return $tmpUsrInfo;
    }
